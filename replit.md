...
**Database Layer**: Prisma ORM with PostgreSQL (Replit Neon). Schema includes User, Club, Team, Venue, Offer, OfferRequest, SavedOffer, GeocodeCache, Conversation, ChatMessage.
...
- **Registration Process**: Two-level age category selection (Junioren, Juniorinnen, Herren, Damen, Freizeitliga) with conditional sub-age options. Year field only shown for Junioren/Juniorinnen teams. Teams stored with both ageCategory and ageGroup for flexible filtering.
- **Navigation**: Redesigned bottom navigation with "Meine Spiele" (My Games) including "Meine Angebote", "Gemerkt", "Angefragt", and "Vereinbart" tabs. Offer creation accessed via prominent orange button in "Meine Angebote". **Header Navigation**: Burger menu with "Offene Bewertungen" and "Meine Statistik" options. Chat icon links to messaging interface.
- **In-App Messaging**: WhatsApp-style peer-to-peer chat system for trainer communication. Features include conversation list, "+" button for new messages, real-time search for clubs and trainers, trainer selection with "Name (Club) AgeGroup" format, message threading, read status tracking, and date-grouped message display. Chat button in header navigates to `/chat`.
